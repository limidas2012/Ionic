...
export class ChattingPage {
  ...
  constructor(navParam: NavParams, private toastCtrl: ToastController,
    ...
    this.chatRef.child(this.chatChild).limitToLast(20).on("child_added", function (snapshot, prevChildKey) {
      ...
      if (newMessage) {
        newMessage.timeDiff = Helper.formatMillisDateTime(Number(newMessage.dateTimeStamp), component.locale);
    ...
    this.translate.get("just_moment").subscribe(value => this.showToast(value));
  }
  ...
  notifyMessage() {
    this.translate.get(['new_msg', 'new_msg_from']).subscribe(value => {
      this.oneSignal.postNotification({
        ...
        contents: { en: ((value['new_msg_from']) + " " + this.userMe.name) },
        data: { msgs: this.messages[this.messages.length - 1] }
  ...
  markDelivered(msg: Message) {
    ...
    //       this.notifyMessage(messagesPendingToNotify);
    //     }
    //   }, 2000);
    // }
  }
